...
INSTRUCTIONS = {
    ...
    "короткий_переход_если_ноль"            : {"code": b"\x74", "type": "short_jcc"},   # синоним
    "короткий_переход_если_не_ноль"         : {"code": b"\x75", "type": "short_jcc"},   # синоним
}
...
def encode_instruction(mnemonic, operands, line_num, line_text):
    ...
    if itype == "none":
        ...
        target = parse_operand(operands[0])
        current_addr = vaddr_text + position[".text"]
        offset = target - (current_addr + 2)  # 1 байт опкода + 1 байт disp8
        if not (-128 <= offset <= 127):
...
def parse_instruction_or_directive(tokens, line_num, line_text):
    ...
    if word.startswith('.'):
        if word == '.текст':
            ...
            for i in range(1, len(tokens)):
                ...
                if pass_num == 2:
                    old_pos = position[current_section]
                    sections[current_section].append(val)
                    position[current_section] += 1
                    new_pos = position[current_section]
                    log("[ПРОХОД 2] Директива .байт: 0x{:02x}".format(val))
                    log("  Адрес: {} (0x{:x})".format(old_pos, old_pos))
                else:
                    position[current_section] += 1
            return
        else:
    ...
    if pass_num == 2:
        ...
        addr_before = position[current_section]
        sections[current_section] += code
        position[current_section] += len(code)
        addr_after = position[current_section]



                # Логируем детали генерации
        hex_bytes = ''.join('\\x{:02x}'.format(b) for b in code)
        log("[ПРОХОД 2] Инструкция: '" + mnemonic + "'")
        log("  Адрес до: " + str(addr_before) + " (0x" + hex(addr_before)[2:] + ")")
        log("  Байты: " + hex_bytes)
        log("  Длина: " + str(len(code)))
        log("  Адрес после: " + str(addr_after) + " (0x" + hex(addr_after)[2:] + ")")
    else:
...
def create_elf(filename):
    ...
    # === ЛОГГИРОВАНИЕ ELF-ЗАГОЛОВКА ===
    log("=== Сформированный ELF-заголовок ===")
    log("e_ident      = " + str(e_ident))
    log("e_type       = " + hex(int.from_bytes(e_type, 'little')))
    log("e_machine    = " + hex(int.from_bytes(e_machine, 'little')))
    log("e_version    = " + hex(int.from_bytes(e_version, 'little')))
    log("e_entry      = " + hex(entry_addr))
    log("e_phoff      = " + hex(int.from_bytes(e_phoff, 'little')))
    log("e_shoff      = " + hex(int.from_bytes(e_shoff, 'little')))
    log("e_flags      = " + hex(int.from_bytes(e_flags, 'little')))
    log("e_ehsize     = " + hex(int.from_bytes(e_ehsize, 'little')))
    log("e_phentsize  = " + hex(int.from_bytes(e_phentsize, 'little')))
    log("e_phnum      = " + hex(int.from_bytes(e_phnum, 'little')))
    log("e_shentsize  = " + hex(int.from_bytes(e_shentsize, 'little')))
    log("e_shnum      = " + hex(int.from_bytes(e_shnum, 'little')))
    log("e_shstrndx   = " + hex(int.from_bytes(e_shstrndx, 'little')))

    # === ЛОГГИРОВАНИЕ PROGRAM HEADER (.text) ===
    log("=== Program Header: .text ===")
    log("p_type       = " + hex(int.from_bytes(p_type, 'little')))
    log("p_flags      = " + hex(int.from_bytes(p_flags, 'little')))
    log("p_offset     = " + hex(int.from_bytes(p_offset, 'little')))
    log("p_vaddr      = " + hex(int.from_bytes(p_vaddr, 'little')))
    log("p_paddr      = " + hex(int.from_bytes(p_paddr, 'little')))
    log("p_filesz     = " + hex(int.from_bytes(p_filesz, 'little')))
    log("p_memsz      = " + hex(int.from_bytes(p_memsz, 'little')))
    log("p_align      = " + hex(int.from_bytes(p_align, 'little')))

    # === ЛОГГИРОВАНИЕ PROGRAM HEADER (.data) ===
    log("=== Program Header: .data ===")
    log("p_type       = " + hex(int.from_bytes(p_type, 'little')))
    log("p_flags      = " + hex(int.from_bytes(p_flags_data, 'little')))
    log("p_offset     = " + hex(int.from_bytes(p_offset_data, 'little')))
    log("p_vaddr      = " + hex(int.from_bytes(p_vaddr_data, 'little')))
    log("p_paddr      = " + hex(int.from_bytes(p_paddr_data, 'little')))
    log("p_filesz     = " + hex(int.from_bytes(p_filesz_data, 'little')))
    log("p_memsz      = " + hex(int.from_bytes(p_memsz_data, 'little')))
    log("p_align      = " + hex(int.from_bytes(p_align, 'little')))

    # === Опционально: дамп байтов заголовков (для бинарной верификации) ===
    def bytes_to_hex_str(b):
        s = ""
        for byte in b:
            s += "\\x" + hex(byte)[2:].zfill(2)
        return s

    log("Байты ELF-заголовка: " + bytes_to_hex_str(elf_header))
    log("Байты program header .text: " + bytes_to_hex_str(text_header))
    log("Байты program header .data: " + bytes_to_hex_str(data_header))
