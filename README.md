...
- **Четырёхпроходная сборка** — традиционная схема с разрешением меток, констант и перекодированием
...
CSV-файл (.csv)              ← содержит все байты программы, метки, заглушки
       │
       ▼ (kvs_pass2.py)      ← ПРОХОД 2: разрешение меток (замена заглушек)
       │
...
| `kvs_lexer.py` | Лексический анализ, разбор на токены (с поддержкой `[]`, `+`, `-`, `*`) |
...
| `kvs_pass1.py` | Первый проход — вычисление размеров секций, адресов меток, генерация CSV с заглушками |
| `kvs_pass2.py` | Второй проход — разрешение меток |
| `kvs_pass3.py` | Третий проход — подстановка констант |
| `kvs_pass4.py` | Четвёртый проход — перекодирование инструкций |
...
2. **Второй проход** — после того как все адреса меток известны, можно заменить заглушки на реальные значения.
3. **Третий проход** — константы могут ссылаться друг на друга и на метки, поэтому их подстановка делается отдельно.
4. **Четвёртый проход** — после подстановки чисел адреса/констант нужно перегенерировать машинный код (разные операнды могут менять длину инструкции).
...
- Инструкции для работы со стеком (PUSH, POP, CALL, RET) — кроме базовых PUSH/POP reg
