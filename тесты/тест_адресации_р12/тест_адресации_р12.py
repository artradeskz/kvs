...
"""
Тест для проверки косвенной адресации через регистр R12
...
SOURCE_REL = SCRIPT_DIR / "тест_адресации_р12.квс"

def clean():
    """Очищает артефакты сборки"""
    files = [
        "тест_адресации_р12.elf",
        "тест_адресации_р12.csv", 
        "тест_адресации_р12.аст",
        "тест_адресации_р12.токены",
        "тест_адресации_р12.константа",
        "тест_адресации_р12_pass4.log"
    ]
    for f in files:
        f_path = SCRIPT_DIR / f
        if f_path.exists():
...
def test_with_char(char: str, expected_code: int) -> bool:
    ...
    elf_path = SCRIPT_DIR / "тест_адресации_р12.elf"
...
def main():
    ...
    print("DEBUG: Начало теста тест_адресации_р12", file=sys.stderr)
    ...
    tests = [
        ...
        ('~', 126),  # Тильда
        (' ', 32),   # Пробел
        ('\t', 9),   # Табуляция (может не сработать из-за экранирования)
    ]
