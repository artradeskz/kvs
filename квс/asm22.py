    ...
    if itype == "none":
        ...
        if base == 4 or base == 12:  # rsp или r12 - требуют SIB
            if rex != 0x40:
                code.append(rex)
            code.append(op)
            code.append((subop << 3) | 0x04)  # ModR/M: mod=00, reg=subop, r/m=100 (SIB required)
            code.append((4 << 3) | (base & 7))  # SIB: scale=00, index=100(no index), base=base&7
...
def parse_instruction_or_directive(tokens, line_num, line_text):
    ...
    if pass_num == 2:
        ...
        size_map = {
            ...
            "pop_mem": 4,              # REX + 0x8F + ModR/M (/0)   КОСЯЧИТ!!!      SIB!!!!
            "in_imm": 2,               # 0xE4 + imm8
            "out_imm": 2,              # 0xE6 + imm8
